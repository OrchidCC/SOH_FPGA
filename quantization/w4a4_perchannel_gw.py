#!/usr/bin/env python3
"""Per-channel ActGridWarp vs per-layer ActGridWarp for W4A4.

Current GridWarp: 1 scalar (scale, bias) per sin layer → shared across all channels.
Proposed: per-channel (scale_j, bias_j) → each neuron gets its own A4 grid alignment.

B:  Per-layer GridWarp + QAT (current baseline)
...
# ─── Per-layer GridWarp (baseline) ────────────────────────────────

class LayerGridWarp(nn.Module):
    """Scalar (scale, bias) shared across all channels."""
...
# ─── Per-channel GridWarp (proposed) ──────────────────────────────

class ChannelGridWarp(nn.Module):
    """Per-channel (scale_j, bias_j) for each neuron."""
    ...
    def init_from_stats(self, act_tensor):
        """Initialize from calibration activations. act_tensor: (N, C)"""
        with torch.no_grad():
            # Per-channel 99.5th percentile of |x| as scale
            ch_abs = act_tensor.abs()
            scale = torch.quantile(ch_abs, 0.995, dim=0).clamp(min=0.05)
            self.log_scale.copy_(torch.log(scale))
            # Per-channel mean as bias
            self.bias.copy_(act_tensor.mean(dim=0))

    def forward(self, x):
        # x: (..., C)
        scale = torch.exp(self.log_scale)  # (C,)
        centered = (x - self.bias) / scale
        qm = 2**(self.a_bits-1)-1
...
class PerChannelModel(nn.Module):
    ...
    def __init__(self):
        ...
        # Per-channel warps: sin1→60ch, sin2→60ch, sin3→32ch
        self.act_warps = nn.ModuleList([
    ...
    def init_warps(self, fp_model, x_calib):
        """Initialize per-channel warp params from FP32 model activations."""
...
def run_one(model_path, dataloader, pinn_alpha, pinn_beta):
    ...
    # B: Per-layer GridWarp (baseline)
    m = BaselineModel(act_stats); m.load_state_dict(ckpt['solution_u'], strict=False)
    ...
    m = PerChannelModel(); m.load_state_dict(ckpt['solution_u'], strict=False)
...
def main():
    ...
    for ds, mdir, load_fn, alpha, beta in tasks:
        ...
        for e in range(1, 11):
            ...
            runs.append(r)
            print(f"FP32={r['fp32']:.2f}  B={r['B']:.2f}  P={r['P']:.2f}  ({time.time()-t0:.0f}s)")
        ...
        if runs:
            ...
            print(f"  MEAN: B={means['B']:.3f}±{stds['B']:.3f}  P={means['P']:.3f}±{stds['P']:.3f}")

        os.makedirs('results', exist_ok=True)
        with open('results/w4a4_perchannel_gw_full.json', 'w') as f:
            json.dump(all_results, f, indent=2, default=float)

    print(f"\n{'='*60}")
    print("B = per-layer GridWarp, P = per-channel GridWarp")
    print(f"{'='*60}")
    for ds, v in all_results.items():
        m = v['mean']; s = v['std']
        print(f"{ds:<12s} FP32={m['fp32']:.2f}  B={m['B']:.2f}±{s['B']:.2f}  P={m['P']:.2f}±{s['P']:.2f}")
